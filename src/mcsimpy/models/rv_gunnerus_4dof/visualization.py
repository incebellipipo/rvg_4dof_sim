...
def draw_ship_2D(eta, parP):
    ...
    if (M1 != M2) or K1 != 3:
        ...
        for ii in range(M1):
            ...
            R = np.array(
                [
                    [np.cos(eta[2, ii]), -np.sin(eta[2, ii])],
                    [np.sin(eta[2, ii]), np.cos(eta[2, ii])],
                ]
            )
            ...
            if parP["type"][ii] == "arrow":
                arrow0 = np.array(
                    [
                        [r1, r1 * np.cos(c2), 0, r1 * np.cos(-c2), r1],
...
def animate_ship_2D(t, eta, parP):
    ...
    if (M0 % 3) != 0 or (M0 < 3) or (N0 != N):
        ...
        ship_ani = anim.FuncAnimation(
            fig0, animate_func, interval=parP["frame_delay"], frames=N
        )
