...
def dot_RVG_Man_4DOF(x, tau, w, parV, parA, parS):
    ...
    nu_c_n_2 = parS["Uc"] * np.array(
        [np.cos(parS["betac"]), np.sin(parS["betac"])]
    ...
    dx[4:8] = invM @ (
        Fact - Fcorr_RB - Fcorr_a - F_visc - D @ nu_r - K @ eta + Ma @ dnu_c + w
    )
